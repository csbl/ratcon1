function model = ncomm_blais_xls2model(fileName,biomassRxnEquation)
% xls2model Writes a model from Excel spreadsheet. This version,
% ncomm_blais_xls2model was modified to resolve some bugs from a previous
...
if isunix
    ...
    [~, Strings, rxnInfo] = xlsread(fileName,'reactions');
    [~, MetStrings, metInfo] = xlsread(fileName,'metabolites');
